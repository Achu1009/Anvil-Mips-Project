...
module tb_mips_pipeline_processor;
    ...
    //-----------------------------------------------------------------
    bind mips_pipeline_processor mips_sva u_sva (
        .clk              (clk),
        .reset            (reset),

        // IF stage  -- pc lives inside the instruction_fetch instance
        .pc               (u_if.pc),          // ADDED (hierarchical)
        .if_instr         (if_instr),         // ADDED
        .if_pc_plus4      (if_pc_plus4),      // ADDED
        ...
        .id_rt            (id_rt),
        .id_opcode        (id_opcode),
        .id_rd1_wire      (id_rd1_wire),      // ADDED
        .id_rd2_wire      (id_rd2_wire),      // ADDED

        // Forwarding selects
        .forward_a        (forward_a),        // ADDED
        .forward_b        (forward_b),        // ADDED
        .ex_forward_a_data(ex_forward_a_data),// ADDED (P33)
        .ex_forward_b_data(ex_forward_b_data),// ADDED (P33)

        // Later pipeline stages
        .ex_mem           (ex_mem),           // ADDED
        .mem_wb           (mem_wb),           // ADDED
        .mem_read_data    (mem_read_data),    // ADDED (P33)
        .wb_data          (wb_data),          // ADDED (P33)

        // Hazard / control
        .load_use_hazard  (load_use_hazard),
        .pc_write         (pc_write),
        .if_id_write      (if_id_write),
        .ex_branch_taken  (ex_branch_taken),  // ADDED
        .ex_branch_target (ex_branch_target), // ADDED
    ...
    //-----------------------------------------------------------------
    bind register_file mips_regfile_sva u_rf_sva (
        .clk        (clk),
        .reset      (reset),
        .reg_write  (reg_write),
        .write_reg  (write_reg),
    ...
    //-----------------------------------------------------------------
    bind data_memory mips_dmem_sva #(.DEPTH(DEPTH)) u_dmem_sva (
        .clk   (clk),
        .reset (reset),
        .req   (req),
        .we    (we),
